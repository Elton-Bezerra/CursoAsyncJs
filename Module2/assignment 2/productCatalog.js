...
document.getElementById("searchType").addEventListener('click', () => {
    processSearch(document.getElementById("type").value, 'type');
});

document.getElementById("searchPrice").addEventListener('click', () => {
    processSearch(document.getElementById("price").value, 'price');
});
...
function processSearch(searchId, type) {
    switch (type) {
        case 'id':
        ...
        case 'type':
            api.searchProductsByType(searchId).then(function (val) {
                var typeArray = val;
                updateTable('similarTable', typeArray);
            }).catch(function (error) {
                alert(error);
            });
            break;
        case 'price':
            api.searchProductsByPrice(searchId, 50).then(function (val) {                
                var priceArray = val;
                updateTable('similarTable', priceArray);
            }).catch(function (error) {
                alert(error);
            });
            break;
